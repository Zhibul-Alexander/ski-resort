...
export default async function Home({ params }: { params: Promise<{ lang: Lang }> }) {
  ...
  return (
    <div className="py-10">
      ...
      <SlideIn index={1}>
        <div id="find-us" className="scroll-mt-[120px] lg:scroll-mt-[86px] mt-10">
          <Section title={site.pageTitles?.findUs || "Find us"} subtitle={site.location.addressLine}>
            <div className="grid gap-6 lg:grid-cols-2 mb-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>{(site.pageTitles as any)?.addresses || "Addresses"}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="flex-1 text-sm">Georgia, Bakuriani, Rental - 1</span>
                      <a href="https://maps.app.goo.gl/wYaNhzUA6QiouFnQA" target="_blank" rel="noreferrer" className="no-underline">
                    ...
                    <div className="flex items-center gap-3">
                      <MapPin className="h-5 w-5 text-accent flex-shrink-0" />
                      <span className="flex-1 text-sm">Georgia, Bakuriani, Rental - 2</span>
                      <a href="https://maps.app.goo.gl/wYaNhzUA6QiouFnQA" target="_blank" rel="noreferrer" className="no-underline">
                        <Button variant="outline" size="sm" className="gap-2">
                          {site.pageTitles?.openInGoogleMaps || "Open in Google Maps"}
                          <ExternalLink className="h-4 w-4" />
                    ...
                    {/* Динамические адреса из конфигурации */}
                    {(site.location as any)?.addresses && (site.location as any).addresses.length > 0 && (
                      <>
                        {(site.location as any).addresses.map((address: any, idx: number) => (
